...
def usage():
    ...
    print('Usage:')
    print('')
    print('$ trigger.py <action> [<argname>=<argvalue> ...]')
    print('')
    print('Actions:')
    ...
    print('  spin: Spin the LEDs until next action; arg {interval} is the optional cycle time (default 1s) and arg {direction} is the spin direction (cw for clockwise or ccw for counterclockwise).')
    print('  hold: Keep LEDs on for a given time unless overridden; ignores "off" commands; arg {holdtime} is the optional time to hold LEDs on.')
    print('')
    print('Args:')
    print('  All keyworded args are optional, including colour which defaults to "white".')
    print('  An arg should be specified as {argname}={argvalue}, for example "colour=white" or "interval=2"; see examples below for more details.')
    print('')
    print('Colours:')
    print('  The following are valid colours for respeaker-led: white red blue green yellow cyan magenta')
    print('')
    print('Examples:')
